...
from rlkit.util import pythonplusplus as ppp
...
class Logger(object):
    def __init__(self):
        ...
        self._earliest_snapshot_epoch = 0
    ...
    def get_earliest_snapshot_epoch(self, ):
        return self._earliest_snapshot_epoch

    def set_earliest_snapshot_epoch(self, epoch):
        self._earliest_snapshot_epoch = epoch
    ...
    def record_tabular_misc_stat(self, key, values, placement='back'):
        ...
        if len(values) > 0:
            self.record_tabular(
                prefix + "Average" + suffix, np.average(values))
    ...
    def dump_tabular(self, *args, **kwargs):
        ...
        if len(self._tabular) > 0:
            ...
            for tabular_fd in list(self._tabular_fds.values()):
                ...
                if wh or (
                        (wh is None) and 
                        (tabular_fd not in self._tabular_header_written)):
                    writer.writeheader()
    ...
    def save_itr_params(self, itr, params):
        if self._snapshot_dir:
            if self._snapshot_mode == 'all':
                ...
                if (itr % self._snapshot_gap == 0 and
                        itr >= self._earliest_snapshot_epoch):
                    file_name = osp.join(self._snapshot_dir, 'itr_%d' % itr)
                    self._save_params_to_file(
                        params, file_name, mode=self._save_param_mode)
...
def setup_logger(
    ...
    logger.set_earliest_snapshot_epoch(earliest_snapshot_epoch)
...
def create_log_dir(
    ...
    if run_id is not None:
        ...
        if variant.get("num_exps_per_instance", 0) > 1:
            ...
            trial_name = (
                "run%s/id%s/%s--s%d" % (run_id, exp_id, timestamp, seed))
        else:
            trial_name = "run{}/id{}".format(run_id, exp_id)
    else:
        trial_name = create_trial_name(
            exp_name, exp_id=exp_id, seed=seed,
